...
import { NormalizedBrowserBuilderSchema, getBrowserLoggingCb } from '@angular-devkit/build-angular/src/browser';
import { ElectronRendererBuilder } from '../electron-renderer';
...
export class ElectronDevServerBuilder implements Builder<DevServerBuilderOptions> {
  ...
  run(builderConfig: BuilderConfiguration<DevServerBuilderOptions>): Observable<BuildEvent> {
    ...
    return checkPort(options.port, options.host).pipe(
      ...
      concatMap(() => {
        ...
        const webpackConfig = electronRendererBuilder.buildWebpackConfig(
